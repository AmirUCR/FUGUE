...
def mycocosm_delimited_ortho(row):
    ...
    for o_gene in ortho_genes:
        o_gene_pid_match = None

        try:
            o_gene_pid_match = o_gene.description.split(' ')[1].split('|')[2]
        except:
            return row['original_name']
    ...
    with open('data/fourdbs_concat/ortho_from_gff/' + ortho_f_name, 'w') as file:
        for g in list(genes):
            gene_pid_match = re.search(r';transcriptId=(.*?)([;\n]|$)', g.description)
            
            if gene_pid_match and gene_pid_match.group(1) in pids:
                count += 1
                found.append(gene_pid_match.group(1))

                idx = pids.index(gene_pid_match.group(1))
                rec = SeqRecord(
                    g.seq,
                    id=gene_pid_match.group(1),
